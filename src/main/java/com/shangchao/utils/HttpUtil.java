...
//        log.info(result);
    ...
    protected static HttpURLConnection getConnection(URL url, String method) throws IOException {
        ...
        if (StringUtils.equals(CommonConstant.URL_PROTOCOL_HTTPS, url.getProtocol())) {
     ...
     * @return 返回状态
     */
    public static Map defaultDownloadaterial(String api, TreeMap<String, String> params, String method,
                                                     String savePath) {
    ...
    protected static String getFileName(String mediaUrl) {
        return StringUtils.substringAfterLast(mediaUrl, CommonConstant.SEPARATOR_SLASH_);
    }
     ...
     * @return
     */
    private static Map contentType(String contentType, HttpURLConnection conn, String savePath) {
        Map result = new HashMap<>(4);
        if (conn == null) {
            result.put("code", CodeAndMsgEnum.ERROR.getcode());
            result.put("msg", "微信素材连接错误！");
            return result;
        }
        try {
            InputStream input = conn.getInputStream();
            ...
            if (StringUtils.equals(contentType, MimeTypes.PLAIN_TEXT) || StringUtils.equals(contentType, MediaType.APPLICATION_JSON_VALUE)) {
                result.put("code", CodeAndMsgEnum.ERROR.getcode());
                result.put("msg", IOUtils.toString(input));
            } else {
     ...
     * @return
     */
    private static Map inputToMedia(InputStream input, String savePath, String type) {
        Map result = new HashMap<>(4);
        ...
        try {
            ...
            result.put("msg", "微信素材保存成功！");
        } catch (IOException e) {
            result.put("code", CodeAndMsgEnum.ERROR.getcode());
            result.put("msg", "微信素材保存失败！");
        }
        return result;
    }
